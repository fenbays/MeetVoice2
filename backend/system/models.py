...
class Role(CoreModel):
    ...
    dept = models.ManyToManyField(to='Dept', verbose_name='数据权限-关联部门', db_constraint=False,
                                  help_text="数据权限-关联部门")
    menu = models.ManyToManyField(to='Menu', verbose_name='关联菜单', db_constraint=False, null=True, blank=True, help_text="关联菜单")
    permission = models.ManyToManyField(to='MenuButton', verbose_name='关联菜单的接口按钮', db_constraint=False, null=True, blank=True,
                                        help_text="关联菜单的接口按钮")
    column = models.ManyToManyField(to='MenuColumnField', verbose_name='列表权限', db_constraint=False, null=True, blank=True,
                                    help_text="列表权限")
...
class File(CoreModel):
    ...
    @classmethod
    def create_from_file(cls, file_obj, name=None):
        """正确的创建方法"""
        # 1. 计算MD5
    ...
    def __str__(self):
        return self.name if self.name else (self.save_name if self.save_name else "未命名文件")
